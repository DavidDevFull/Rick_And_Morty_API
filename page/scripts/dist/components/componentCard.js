export default class ComponentCard {
    tagFather;
    information;
    constructor(tagFather, information) {
    ...
    getStyle(value) {
        return `style="color: ${ComponentCard.colors[value] || ComponentCard.colors.unknown}"`;
    }
    translate(group, value) {
        return ComponentCard.translations[group]?.[value.toLowerCase()] || value;
    }
    cardCharacter() {
        const { image, name, id, gender, species, status, origin } = this.information;
